...
public class AuthBLTests
{
    ...
    [Fact]
    public async Task LoginAsync_Unknown_User_Should_Throw_ValidationException_Without_Exposing_User_Existence()
    {
    ...
    [Fact]
    public async Task LoginAsync_Inactive_User_Should_Throw_ValidationException()
    {
    ...
    [Fact]
    public async Task LoginAsync_Locked_User_Should_Throw_ValidationException()
    {
        var user = CreateTestUser(isActive: true);
        _fakeDac.Users[user.Email.ToUpperInvariant()] = new UserSecurityInfoBO
        {
    ...
    private sealed class FakePasswordVerifier : IPasswordVerifier
    {
        ...
        public PasswordVerificationResult VerifyPassword(string hashedPassword, string providedPassword) => NextResult;
    }
